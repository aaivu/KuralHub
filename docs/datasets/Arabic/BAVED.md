# 🎙️ BAVED - Arabic

🔹 **Dataset Name:** Basic Arabic Vocal Emotions Dataset  <br/> 
🔹 **Language:** Arabic  <br/> 
🔹 **Dataset Type:** 🎙️ Audio <br/>
🔹 **Emotion Categories:** Low, Neutral, High  <br/> 
🔹 **Size:** Number of samples 1935, speakers: 61 <br/> 
🔹 **Recording Environment:** 🎭 Acted  <br/> 
🔹 **License:** Not Specified <br/> 
🔹 **Access:** 🔗 Open <br/> 

---

## 📜 **Dataset Overview**
Basic Arabic Vocal Emotions Dataset (BAVED) is a dataset that contains an arabic words spelled in diffrent levels of expressed emotions recorded in an audio/wav format.

---

## 📥 **Access & Download**
🔗 **Official Website:** Not Specified  <br/> 
📄 **Paper:** Not Specified <br/> 
🗂️ **Download Link:** [GitHub](https://github.com/40uf411/Basic-Arabic-Vocal-Emotions-Dataset) <br/> 
💾 **File Format:** `.wav`  <br/> 
📧 Contact: Not Specified </br>
> ⚠️ **Note:** Fill the request process if the dataset is restricted.

---

## 📜 **Citation**
If you're using this dataset, please cite both the original authors and our survey paper, as it contributed to making the dataset accessible.


```
@misc{aouf2019baved,
  title        = {Basic Arabic Vocal Emotions Dataset (BAVED)},
  author       = {Ali Aouf},
  year         = {2019},
  publisher    = {GitHub},
  howpublished = {\url{https://github.com/40uf411/Basic-Arabic-Vocal-Emotions-Dataset}},
  note         = {Accessed: 2025-05-10}
}
```

```
TBD - Need to add github as reference
```

---

🔍 **Back to [All Arabic Datasets](Readme.md)**  <br/> 
📩 **Found an issue? [Submit an update](../../CONTRIBUTING.md)!**
