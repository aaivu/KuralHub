# 🎙️ HESD - Hungarian

🔹 **Dataset Name:** Hungarian Emotional Speech Database  <br/> 
🔹 **Language:** Hungarian  <br/> 
🔹 **Dataset Type:** 🎙️ Audio  <br/>
🔹 **Emotion Categories:** Fear, Sadness, Anger, Surprise, Disgust, Joy, Excitement, Neutral  <br/> 
🔹 **Size:** Number of samples: 760 , speakers: 38  <br/> 
🔹 **Recording Environment:** 🎭 Acted(Non Professional Speakers - Quiet Office Room)  <br/> 
🔹 **License:** Not Specified  <br/> 
🔹 **Access:** 🔒 Request <br/> 

---

## 📜 **Dataset Overview**
This dataset comprises recordings of three Hungarian sentences, each articulated to express eight fundamental emotions: happiness, sadness, anger, fear, surprise, disgust, contempt, and neutral. 
These recordings were performed by nonprofessional speakers. 
The primary objective of this dataset is to facilitate the examination of human emotion perception and to support the development of automatic emotion recognition systems. 
To achieve this, the dataset was utilized in perception tests comparing human recognition of emotions and in training Hidden Markov Models (HMM) for automatic emotion recognition, optimizing parameters such as acoustic preprocessing and the number of states in the models.

---

## 📥 **Access & Download**
🔗 **Official Website:** Not Specified  <br/> 
📄 **Paper:** [Speech Emotion Perception by Human and Machine](https://www.researchgate.net/profile/Klara-Vicsi/publication/225204892_Speech_Emotion_Perception_by_Human_and_Machine/links/09e4150b71db991837000000/Speech-Emotion-Perception-by-Human-and-Machine.pdf) <br/> 
🗂️ **Download Link:** Not Specified <br/> 
💾 **File Format:**  `.wav`  <br/> 
📧 Contact: [toth.sz@tmit.bme.hu](mailto:toth.sz@tmit.bme.hu?subject=Dataset%20Access%20Request), [vicsi@tmit.bme.hu](mailto:vicsi@tmit.bme.hu?subject=Dataset%20Access%20Request), [sztaho.david@tmit.bme.hu](mailto:sztaho.david@tmit.bme.hu?subject=Dataset%20Access%20Request) </br>
> ⚠️ **Note:** Fill the request process if the dataset is restricted.

---

## 📜 **Citation**
If you're using this dataset, please cite both the original authors and our survey paper, as it contributed to making the dataset accessible.

```
@inproceedings{toth2008speech,
  title={Speech emotion perception by human and machine},
  author={T{\'o}th, Szabolcs Levente and Sztah{\'o}, David and Vicsi, Kl{\'a}ra},
  booktitle={Verbal and Nonverbal Features of Human-Human and Human-Machine Interaction: COST Action 2102 International Conference, Patras, Greece, October 29-31, 2007. Revised Papers},
  pages={213--224},
  year={2008},
  organization={Springer}
}
```

---

🔍 **Back to [All Hungarian Datasets](Readme.md)**  <br/> 
📩 **Found an issue? [Submit an update](../../CONTRIBUTING.md)!**
