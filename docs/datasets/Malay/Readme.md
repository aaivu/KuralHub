# 🗂️ Speech Emotion Recognition (SER) Datasets for Malay

Welcome to the **Malay** section of KuralHub!  
This directory contains **Speech Emotion Recognition (SER) datasets** available for **Malay**, along with metadata and access information.

## 🌍 **Language Information**  
- **🗣 Language Name:** Malay  
- **🌐 Language Family:** Austronesian  
- **🔤 ISO Code:** ms  
- **📍 Major Spoken Countries:** Malaysia, Indonesia, Brunei, Singapore, Thailand  

---

## 📜 **Available Datasets**

| Dataset Name | Type       | # Speakers | # Samples | License    | Access  | Research Paper         | Dataset Access |
|--------------|------------|------------|-----------|------------|---------|------------------------|---------------------|
| [Emolah](Emolah.md) | 🎙️ Audio   |    -        | -       | Creative Commons Attribution License  | 🔒 Request | [Link to Paper](https://izzadramli.uitm-apps.com/wp-content/uploads/2021/03/Emolah_A_Malay_Language_Spontaneous_Speech_Emotion.pdf) | Not Specified |
<!-- | [Dataset 2](dataset2.md) | 🎭 Multimodal | A          | B         | Restricted | 🔗 Open | [Link to Paper](paper2.md) | [Access Link](access2.md) | -->

---

## 📥 **How to Use These Datasets**
1. Browse individual dataset details in their respective `.md` files.
2. Follow access instructions to download or request datasets.
3. Check licensing before using for research or commercial purposes.

---

## 📌 **Contribute**
If you know of **missing datasets** for Malay, feel free to submit an update!  
📩 **[Contribution Guidelines](../../CONTRIBUTING.md)**

---

🚀 **Back to [All Languages](../README.md)**  
