# 🗂️ Speech Emotion Recognition (SER) Datasets for Swedish

Welcome to the **Swedish** section of KuralHub! <br/>
This directory contains **Speech Emotion Recognition (SER) datasets** available for **Swedish**, along with metadata and access information.

## 🌍 **Language Information**

- **🗣 Language Name:** Swedish <br/>
- **🌐 Language Family:** Indo-European <br/>
- **🔤 ISO Code:** sv <br/>
- **📍 Major Spoken Countries:** Finland, Sweden, Åland, European Union, Nordic Council <br/>

---

## 📜 **Available Datasets**

| Dataset Name             | Type          | # Speakers | # Samples | License    | Access     | Research Paper             | Dataset Access            |
| ------------------------ | ------------- | ---------- | --------- | ---------- | ---------- | -------------------------- | ------------------------- |
| [EU-Emotion-Voice-Database](EU-Emotion-Voice-Database.md) | 🎙️ Audio      | 54         | 2,159     | Not Specified | 🔒 Request | [Link to Paper](https://link.springer.com/content/pdf/10.3758/s13428-018-1048-1.pdf) | Not Specified |
| [Swedish Voice Provider Emotional Speech Corpus](SwedishVoiceProviderEmotionSpeechCorpus.md) | 🎙️ Audio | Not Specified | ~10,000 utterances | Not Specified | 🔒 Request | [Link to Paper](https://www.cs.cmu.edu/~kornel/pubs/is061581.pdf) | Not Specified |

---

## 📥 **How to Use These Datasets**

1. Browse individual dataset details in their respective `.md` files.
2. Follow access instructions to download or request datasets.
3. Check licensing before using for research or commercial purposes.

---

## 📌 **Contribute**

If you know of **missing datasets** for Swedish, feel free to submit an update! <br/>
📩 **[Contribution Guidelines](../../CONTRIBUTING.md)**

---

🚀 **Back to [All Languages](../../README.md)**
