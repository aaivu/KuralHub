# 🗂️ Speech Emotion Recognition (SER) Datasets for Japanese

Welcome to the **Japanese** section of KuralHub! <br/>
This directory contains **Speech Emotion Recognition (SER) datasets** available for **Japanese**, along with metadata and access information.

## 🌍 **Language Information**

- **🗣 Language Name:** Japanese <br/>
- **🌐 Language Family:** Japonic <br/>
- **🔤 ISO Code:** ja <br/>
- **📍 Major Spoken Countries:** Japan <br/>

---

## 📜 **Available Datasets**

| Dataset Name             | Type          | # Speakers | # Samples | License    | Access     | Research Paper             | Dataset Access            |
| ------------------------ | ------------- | ---------- | --------- | ---------- | ---------- | -------------------------- | ------------------------- |
| [JVNV](JVNV.md) | 🎙️ Audio      | 4          | 1615         | CC-BY-SA 4.0  | 🔗 Open    | [Link to Paper](https://arxiv.org/pdf/2310.06072) | [Access Link](https://sites.google.com/site/shinnosuketakamichi/research-topics/jvnv_corpus) |
| [Keio-ESD](Keio-ESD.md) | 🎙️ Audio | -         | -         | - | 🔗 Open | - | [Access Link](https://research.nii.ac.jp/src/en/Keio-ESD.html) |

---

## 📥 **How to Use These Datasets**

1. Browse individual dataset details in their respective `.md` files.
2. Follow access instructions to download or request datasets.
3. Check licensing before using for research or commercial purposes.

---

## 📌 **Contribute**

If you know of **missing datasets** for Japanese, feel free to submit an update! <br/>
📩 **[Contribution Guidelines](../../CONTRIBUTING.md)**

---

🚀 **Back to [All Languages](../../README.md)**
