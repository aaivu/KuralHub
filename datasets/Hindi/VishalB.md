# 🎙️ VishalB - Hindi

🔹 **Dataset Name:** Vishal B. (2021). Speech Emotion Recognition (Hindi) Dataset. Kaggle. </br>
🔹 **Language:** Hindi </br>
🔹 **Dataset Type:** 🎙️ Audio </br>
🔹 **Emotion Categories:** Anger, Disgust, Fear, Happy, Neutral, Sad, Sarcastic and Surprise </br>
🔹 **Size:** samples - 3200 </br>
🔹 **Recording Environment:** 🎭 Acted </br>
🔹 **License:** Not Specified </br>
🔹 **Access:** 🔗 Open </br>

---

## 📜 **Dataset Overview**
This is a dataset found on Kaggle for Hindi Speech Emotion with 3,200 samples.

---

## 📥 **Access & Download**
🔗 **Official Website:** [Dataset Homepage](https://github.com/aaivu/KuralNet/blob/add/lux/hindi/datasets/Hindi/VishalB.md)   </br>
📄 **Paper:** Not Available </br>
🗂️ **Download Link:** [Download](https://github.com/aaivu/KuralNet/blob/add/lux/hindi/datasets/Hindi/VishalB.md)  </br>
💾 **File Format:** `.wav`  </br>
📧 Contact:Not Found </br>

---

## 📜 **Citation**
If you're using this dataset, please cite both the original authors and our survey paper, as it contributed to making the dataset accessible.

```
@misc{vishal2021speech,
  author       = {Vishal B.},
  title        = {Speech Emotion Recognition (Hindi)},
  year         = {2021},
  publisher    = {Kaggle},
  url          = {https://www.kaggle.com/datasets/vishlb/speech-emotion-recognition-hindi}
}
```

```
TBD
```

---

🔍 **Back to [All Hindi Datasets](Readme.md)**  
📩 **Found an issue? [Submit an update](../../CONTRIBUTING.md)!**
