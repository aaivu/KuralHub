# 🗂️ Speech Emotion Recognition (SER) Datasets for Vietnamese 

Welcome to the **Vietnamese** section of KuralHub!  
This directory contains **Speech Emotion Recognition (SER) datasets** available for **Vietnamese**, along with metadata and access information.

## 🌍 **Language Information**  
- **🗣 Language Name:** Vietnamese   
- **🌐 Language Family:** Austro-Asiatic
- **🔤 ISO Code:** vi  
- **📍 Major Spoken Countries:** Vietnam

---

## 📜 **Available Datasets**

| Dataset Name | Type       | # Speakers | # Samples | License    | Access  | Research Paper         | Dataset Access |
|--------------|------------|------------|-----------|------------|---------|------------------------|---------------------|
| [VNEMOS](VNEMOS.md) | 🎙️ Audio   |       -    | 250        | -  | 🔒 Request | [Link to Paper](https://www.arxiv.org/pdf/2412.08683) | - |

---

## 📥 **How to Use These Datasets**
1. Browse individual dataset details in their respective `.md` files.
2. Follow access instructions to download or request datasets.
3. Check licensing before using for research or commercial purposes.

---

## 📌 **Contribute**
If you know of **missing datasets** for Vietnamese, feel free to submit an update!  
📩 **[Contribution Guidelines](../../CONTRIBUTING.md)**

---

🚀 **Back to [All Languages](../../README.md)**  
